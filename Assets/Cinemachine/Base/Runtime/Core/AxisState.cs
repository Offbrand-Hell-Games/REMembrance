...
namespace Cinemachine
{
    ...
    public struct AxisState
    {
		...
		[Tooltip("Rewired Player Number")]
		public int m_RewiredPlayerNumber;
        ...
        public AxisState(
            float maxSpeed, float accelTime, float decelTime, float val, string name, bool invert, int playerNum)
        {
            m_MaxSpeed = maxSpeed;
            ...
            mMaxValue = 0f;
            mWrapAround = false;
			m_RewiredPlayerNumber = playerNum; player = null;
        }
        ...
        public bool Update(float deltaTime)
        {
			if(Application.isPlaying)
			{
				if (player == null)
				{
					player = ReInput.players.GetPlayer(m_RewiredPlayerNumber);
				}
				if (!string.IsNullOrEmpty(m_InputAxisName))
				{
					try
					{
						m_InputAxisValue = player.GetAxis(m_InputAxisName);
					}
