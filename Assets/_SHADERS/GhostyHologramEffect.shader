...
Shader "FX/Ghosty Hologram Effect" {
	Properties{
...
//		float glitchPosClamped = step(0, glitchPos) * step(glitchPos, 0.2);// clamped segment of model
        _CurrHeight = -sin (12.56 * _GlitchTime * o.vertex.y + _Time.w) * _WaveSize * cos(o.vertex.y*2);
...
//
		half rim = 1.0 - saturate(dot(viewDirection, i.normalDir));// rimlight based on view and normal
		fixed4 col = text + cos(rim) * _RimColor * .3 + _RimColor.b * .3;// end result color 
